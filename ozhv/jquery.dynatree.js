...
//      if( !data.uniqueID ){
		if( data.uniqueID == null ){ // test for null OR undefined (issue 420)
			data.uniqueID = "_" + tree._nodeCount++;
		}else{
			data.uniqueID = "" + data.uniqueID; // issue 371
		}
	...
	toString: function() {
		return "DynaTreeNode<" + this.data.uniqueID + ">: '" + this.data.title + "'";
	},
...
//      this.tree.logDebug("%s.render(%s)", this, useEffects);
		...
		if( !parent && !this.ul ) {
			...
			if( ! this.li ) {
				...
				this.li.dtnode = this;
				if( data.uniqueID && opts.generateIds ){
					this.li.id = opts.idPrefix + data.uniqueID;
				}
...
//          logMsg("isHidden:%s", isHidden);
	...
	getKeyPath: function(excludeSelf) {
		...
		this.visitParents(function(node){
			if(node.parent){
				path.unshift(node.data.uniqueID);
			}
		}, !excludeSelf);
...
//      this.tree.logDebug("dtnode.focus(): %o", this);
	...
	_activate: function(flag, fireEvents) {
		...
		if( flag ) {
			...
			if( opts.persist ){
				$.cookie(opts.cookieId + "-active", this.data.uniqueID, opts.cookie);
			}
			this.tree.persistence.activeKey = this.data.uniqueID;
...
//        this.tree._changeNodeList("select", this, sel);
		...
		if( sel ) {
			if( opts.persist ){
				this.tree.persistence.addSelect(this.data.uniqueID);
			}
			...
			if( opts.persist ){
				this.tree.persistence.clearSelect(this.data.uniqueID);
			}
...
//      this.tree.logDebug("dtnode.toggleSelect() - %o", this);
	...
	_expand: function(bExpand, forceSync) {
		...
		if( opts.persist ) {
			if( bExpand ){
				this.tree.persistence.addExpand(this.data.uniqueID);
			}else{
				this.tree.persistence.clearExpand(this.data.uniqueID);
			}
		}
...
//      this.tree.logDebug("dtnode._onFocus(%o): %o", event, this);
		...
		if ( event.type == "blur" || event.type == "focusout" ) {
			...
			if( opts.persist ){
				$.cookie(opts.cookieId + "-focus", this.data.uniqueID, opts.cookie);
			}
		}
...
//      this.tree.logDebug ("%s.remove()", this);
	...
	removeChild: function(tn) {
		...
		if( this.tree.options.persist ) {
			if( tn.bSelected ){
				this.tree.persistence.clearSelect(tn.data.uniqueID);
			}
			if ( tn.bExpanded ){
				this.tree.persistence.clearExpand(tn.data.uniqueID);
			}
		}
...
//          tree.logDebug("init from cookie, pa=%o, dk=%o", pers.activeKey, dtnode.data.uniqueID);
			if( pers.activeKey === dtnode.data.uniqueID ){
				tree.activeNode = dtnode;
			}
			if( pers.focusedKey === dtnode.data.uniqueID ){
				tree.focusNode = dtnode;
			}
			dtnode.bExpanded = ($.inArray(dtnode.data.uniqueID, pers.expandedKeyList) >= 0);
			dtnode.bSelected = ($.inArray(dtnode.data.uniqueID, pers.selectedKeyList) >= 0);
//          tree.logDebug("    key=%o, bSelected=%o", dtnode.data.uniqueID, dtnode.bSelected);
		} else {
...
//          tree.logDebug("init from data");
			if( dtnode.data.activate ) {
				tree.activeNode = dtnode;
				if( opts.persist ){
					pers.activeKey = dtnode.data.uniqueID;
				}
			}
			if( dtnode.data.focus ) {
				tree.focusNode = dtnode;
				if( opts.persist ){
					pers.focusedKey = dtnode.data.uniqueID;
				}
			}
			dtnode.bExpanded = ( dtnode.data.expand === true ); // Collapsed by default
			if( dtnode.bExpanded && opts.persist ){
				pers.addExpand(dtnode.data.uniqueID);
			}
...
*/
			if( dtnode.bSelected && opts.persist ){
				pers.addSelect(dtnode.data.uniqueID);
			}
		}
...
//      this.tree.logDebug("%s.addChild(%o, %o)", this, obj, beforeNode);
	...
	appendAjax: function(ajaxOptions) {
		...
		var orgSuccess = ajaxOptions.success,
			...
			eventType = "nodeLoaded.dynatree." + this.tree.$tree.attr("id") + "." + this.data.uniqueID;
...
//      this.logDebug("reloadAjax: key=%o, an.uniqueID:%o", pers.activeKey, this.activeNode?this.activeNode.data.uniqueID:"?");
...
//      }
	...
	serializeArray: function(stopOnParents) {
		...
		for(var i=0, l=nodeList.length; i<l; i++){
			arr.push({name: name, value: nodeList[i].data.uniqueID});
		}
		return arr;
	},
...
//      this.logDebug("reactivate %o", node);
	...
	loadKeyPath: function(keyPath, callback) {
		...
		if(segList[0] == this.tnRoot.data.uniqueID){
...
$.ui.dynatree.nodedatadefaults = {
	...
	uniqueID: null, // May be used with activate(), select(), find(), ...
